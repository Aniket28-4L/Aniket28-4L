...
import {Composition, AbsoluteFill, interpolate, spring, useCurrentFrame, useVideoConfig} from 'remotion';
...
const Scene = () => {
  ...
  const scenes = [
    {k:'01', title:'AI RECRUITER', sub:'structured candidate evaluation', nodes:['RESUME','PARSER','MATCH','RISK','DECISION']},
    {k:'02', title:'AI OPS COPILOT', sub:'intent → plan → action → memory', nodes:['INPUT','INTENT','PLAN','EXECUTE','MEMORY']},
    {k:'03', title:'DELIVERY', sub:'software that actually ships', nodes:['CODE','GIT','CI/CD','DOCKER','AWS']},
  ];
  ...
  return <AbsoluteFill style={{background:BG,color:FG,fontFamily:'Arial',overflow:'hidden'}}>
    ...
    <div style={{position:'absolute',left:70,top:380,display:'flex',alignItems:'center'}}>
      {s.nodes.map((node,i)=> <React.Fragment key={node}>
        <div style={{width:145,height:76,border:`1px solid ${i === Math.floor((local/80)*5) ? ACCENT : GRID}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,letterSpacing:2,color:i === Math.floor((local/80)*5) ? FG : MUTED,background:'#10100f'}}>{node}</div>
    ...
    <Box x={350} y={540} w={260} h={130} label="STAGE" value={`${String(iPad(scene+1)).padStart(2,'0')} / 03`} />
...
const iPad = (n:number) => n;

export const RemotionRoot: React.FC = () => (
  <Composition id="ProfileMotion" component={Scene} durationInFrames={240} fps={30} width={1000} height={760}/>
);
